...
export function buildHeartbeatPrompt(
    ...
    const parts: string[] = [
        ...
        tierDirective,
        "",
        `Read HEARTBEAT.md from your working directory (${config.cwd}/HEARTBEAT.md).`,
        `If it doesn't exist, create it with sections: Quick Tasks, Hourly Tasks, Daily Tasks, Urgent Flags, Notes.`,
        "",
        `Your heartbeat timing state is in \`${config.cwd}/.borg/heartbeat-state.json\` (read-only — timing is managed automatically).`,
        "If your HEARTBEAT.md has a `## Timestamps` section, remove it — timing is now managed automatically.",
        "",
        "**Key file paths (check these in your Quick/Hourly/Daily tasks):**",
        `- HEARTBEAT.md: ${config.cwd}/HEARTBEAT.md`,
        `- .borg/settings.json: ${config.cwd}/.borg/settings.json`,
        `- .borg/threads.json: ${config.cwd}/.borg/threads.json`,
        `- .borg/message-history.jsonl: ${config.cwd}/.borg/message-history.jsonl`,
        `- .borg/queue/: ${config.cwd}/.borg/queue/`,
        "",
