...
**Cleanup:** No proactive per-message cleanup — images stay at their unique paths and are available for retries or re-reads. `startPeriodicCleanup()` sweeps `.borg/images/` and `.borg/images/incoming/` every 15 minutes, removing files older than 48 hours.
